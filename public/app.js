...
const applyProductCatalog = (root) =>
...
const refreshProductCatalog = async () => {
  ...
  if (cached.length) {
    productCatalog = sortProducts(cached);
  ...
  try {
    ...
    if (
      ...
      products.some((product) => !product?.id || !product?.sku || !product?.name)
    )
      return;
    productCatalog = sortProducts(products);
    await clearProducts();
    await Promise.all(productCatalog.map(saveProduct));
    applyProductCatalog(document);
  } catch {}
};
const resolveBarcode = (input) => {
  ...
  const result = item.querySelector("[data-barcode-result]"),
    product = item.querySelector("[name=product_id]"),
    sku = normalizeSku(input.value);
...
const startCameraScan = async (button) => {
  const item = button.closest(".line-item"),
    input = item?.querySelector("[data-barcode-input]"),
    result = item?.querySelector("[data-barcode-result]"),
    scanner = item?.querySelector("[data-barcode-scanner]"),
    video = scanner?.querySelector("[data-barcode-video]"),
...
const refreshStatuses = async () => {
  const registration = await navigator.serviceWorker.ready;
  registration.active?.postMessage({ type: "REFRESH_REPORT_STATUSES" });
};
...
const submitOfflineFirst = async (event) => {
  ...
  const form = event.currentTarget,
    errors = document.querySelector("#form-errors"),
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.querySelector("#add-line-item")?.addEventListener("click", () => {
    ...
    applyProductCatalog(destination);
  });
