...
export class ReportWorkflow implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "POST" && path === "/initialize")
      return this.initialize(
        (await request.json()) as { reportId: string; storeId: string; totalAmountCents: number },
        correlationId,
      );
    if (request.method === "POST" && path === "/decision")
      return this.decide(
    ...
    if (request.method === "GET" && path === "/state")
      return json(
        (await this.state.storage.get<WorkflowState>("workflow")) ?? { error: "workflow_not_initialized" },
        (await this.state.storage.get("workflow")) ? 200 : 404,
      );
    return json({ error: "not_found" }, 404);
  }

  private async initialize(
    input: { reportId: string; storeId: string; totalAmountCents: number },
  ...
  private async decide(
    input: { role: ApprovalRole; actor: string; decision: "approve" | "reject"; reason?: string },
    correlationId: string,
  ) {
