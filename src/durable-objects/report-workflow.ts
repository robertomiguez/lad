...
type Role = "regional_manager" | "quality";
type WorkflowState = { reportId: string; storeId: string; totalAmountCents: number; status: "pending_regional" | "pending_quality" | "approved" | "rejected"; escalated?: boolean; escalationTargetRole?: Role; erpEnqueued?: boolean };
...
export class ReportWorkflow implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "POST" && path === "/decision") return this.decide(await request.json() as { role: Role; actor: string; decision: "approve" | "reject"; reason?: string }, correlationId);
  ...
  private async initialize(input: { reportId: string; storeId: string; totalAmountCents: number }, correlationId: string) {
    ...
    await this.transition(workflow, correlationId, "system", "submitted");
    if (status.startsWith("pending_")) await this.scheduleEscalation();
    return json(workflow, 201);
  }

  private async decide(input: { role: Role; actor: string; decision: "approve" | "reject"; reason?: string }, correlationId: string) {
    ...
    const correctRole = (current.status === "pending_regional" && input.role === "regional_manager") || (current.status === "pending_quality" && input.role === "quality");
    ...
    current.status = current.status === "pending_regional" ? statusAfterRegionalApproval(current.totalAmountCents) : "approved";
    current.escalated = false;
    current.escalationTargetRole = undefined;
    await this.transition(current, correlationId, input.actor, previousStatus);
    if (current.status === "pending_quality") await this.scheduleEscalation(); else await this.state.storage.deleteAlarm();
    return json(current);
  }

  async alarm() {
    const current = await this.state.storage.get<WorkflowState>("workflow");
    if (!current || !current.status.startsWith("pending_") || current.escalated) return;
    ...
    current.escalationTargetRole = "quality";
  ...
  private async transition(workflow: WorkflowState, correlationId: string, actor: string, fromStatus: string, rejectionReason?: string) {
    ...
    if (workflow.status === "approved" && !workflow.erpEnqueued) {
