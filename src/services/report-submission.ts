...
export async function createReport(request: Request, env: Env, claims: Claims, correlationId: string) {
  ...
  const existing = kvHit ? await reports.findExistingForStore(reportId, claims.store_id) : await reports.hasIdempotencyKey(reportId) ? await reports.findExistingForStore(reportId, claims.store_id) : null;
