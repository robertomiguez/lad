...
export async function loginPage(env: Env) {
  const { results } = await env.DB.prepare("SELECT id, name, role FROM users ORDER BY CASE role WHEN ? THEN 1 WHEN ? THEN 2 WHEN ? THEN 3 ELSE 4 END, name").bind(ROLE.store, ROLE.regionalManager, ROLE.quality).all<Pick<DbUser, "id" | "name" | "role">>();
